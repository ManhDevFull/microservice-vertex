...
RUN dotnet restore dotnet/dotnet.csproj \
    ...
    && dotnet restore ShippingService/ShippingService.csproj \
...
RUN dotnet publish dotnet/dotnet.csproj -c $BUILD_CONFIGURATION -o /out/dotnet \
    ...
    && dotnet publish ShippingService/ShippingService.csproj -c $BUILD_CONFIGURATION -o /out/shipping \
    && dotnet publish PaymentService/PaymentService.csproj -c $BUILD_CONFIGURATION -o /out/payment
...
ENV DOTNET_GRPC=http://127.0.0.1:5100 \
    ...
    GRPC__SHIPPINGURL=http://127.0.0.1:5306 \
    GRPC__PAYMENTURL=http://127.0.0.1:5308 \
    PORT=5200
